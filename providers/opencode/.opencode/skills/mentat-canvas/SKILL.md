...
2. `start_operation_run`. A refusal naming operations that have not run is the prerequisite check:
   ...
   STOP and call the question tool to clarify. Never set `forcePastPrerequisites` on your own judgement.
...
on the plan, and call `apply_pivot` only on their instruction. STOP and call the question tool to clarify.
